#!/bin/bash
# Batch publish Skills to ClawHub API - Corrected Format
...
publish_skill() {
    ...
    echo "Publishing: $skill_name ($display_name)"
    ...
    # Prepare JSON payload with correct structure
    local payload
    payload=$(cat <<EOF
{
    "name": "$skill_name",
...
EOF
    ...
    case "$http_code" in
        ...
        *)
            ...
            echo "{\"name\":\"$skill_name\",\"status\":\"failed\",\"code\":$http_code,\"error\":\"$(echo "$body" | sed 's/"/\\"/g')\"}" 
            ;;
    esac
}
...
# Publish each skill
echo "Starting batch publish of Skills to ClawHub (Corrected Format)..."
