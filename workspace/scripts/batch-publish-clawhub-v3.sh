#!/bin/bash
# Batch publish Skills to ClawHub API - With SHA256
...
# Function to convert skill name to display name
to_display_name() {
    echo "$1" | sed 's/-/ /g' | sed 's/\b\w/\u&/g' | sed 's/Pro$/ Pro/'
}
...
publish_skill() {
    local filepath="$1"
    local skill_name="$2"
    local display_name=$(to_display_name "$skill_name")
    local file_sha256=$(compute_sha256 "$filepath")
    ...
    # Prepare JSON payload with sha256 but without storageId (let API handle storage)
    local payload
    payload=$(cat <<EOF
{
    "name": "$skill_name",
...
EOF
    ...
    case "$http_code" in
        ...
        403)
            ...
            ;;
        *)
            echo "  ✗ Failed ($http_code): $(echo "$body" | head -c200)"
            echo "{\"name\":\"$skill_name\",\"status\":\"failed\",\"code\":$http_code,\"error\":\"$(echo "$body" | sed 's/"/\\"/g' | head -c150)\"}" 
            ;;
    esac
}
...
# Publish each skill
echo "Starting batch publish of Skills to ClawHub (With SHA256)..."
...
for skill_name in "${!SKILL_FILES[@]}"; do
    ...
    echo "[$count/${#SKILL_FILES[@]}] Processing: $skill_name"
...
# Print summary
echo ""
echo "✅ Successfully Published (${#SUCCESS_LIST[@]}):"
for item in "${SUCCESS_LIST[@]}"; do
...
echo ""
echo "⚠️ Already Existed (${#EXISTING_LIST[@]}):"
for item in "${EXISTING_LIST[@]}"; do
    name=$(echo "$item" | grep -o '"name":"[^"]*"' | cut -d'"' -f4)
    echo "  • $name"
done
...
for item in "${FAILED_LIST[@]}"; do
    name=$(echo "$item" | grep -o '"name":"[^"]*"' | cut -d'"' -f4)
    error=$(echo "$item" | grep -o '"error":"[^"]*"' | cut -d'"' -f4 | cut -c1-80)
    echo "  • $name - $error"
done
